...
def generate_strategic_explanation(
    ...
    if signal == "OPPORTUNITY":
        return (
            f"📈 **Strategic Opportunity Identified for {company}.**\n\n"
            f"The competitive index is strong at **{competitive_index}/100**, "
            f"supported by positive market sentiment ({pos} positive articles) "
            f"and favorable price momentum. The forecast also indicates an upward trend.\n\n"
            f"⚠️ Volatility at **{volatility:.2f}%** suggests moderate risk, "
            f"so opportunities should be pursued with disciplined monitoring."
        )

    if signal == "THREAT":
        return (
            f"📉 **Strategic Risk Detected for {company}.**\n\n"
            f"The competitive index is weak at **{competitive_index}/100**, "
            f"with dominant negative sentiment ({neg} negative articles) "
            f"and declining recent price movement.\n\n"
            f"⚠️ Elevated volatility (**{volatility:.2f}%**) increases downside risk. "
            f"Risk mitigation and close monitoring are advised."
        )

    return (
        f"⚖️ **Mixed Market Signals for {company}.**\n\n"
        f"The competitive index stands at **{competitive_index}/100**, "
        f"indicating no clear directional dominance. Sentiment and price signals "
        f"are currently balanced.\n\n"
        f"👀 Volatility at **{volatility:.2f}%** suggests uncertainty. "
        f"Maintaining observation until stronger confirmation is recommended."
    )
...
def build_strategy_report(
    ...
    return f"""
REAL-TIME MARKET INTELLIGENCE REPORT
===================================
...
STRATEGIC SUMMARY
-----------------------------------
Competitive Positioning Index: {competitive_index}/100
...
SENTIMENT OVERVIEW
-----------------------------------
Positive News: {sentiment_counts.get("positive", 0)}
Neutral News: {sentiment_counts.get("neutral", 0)}
Negative News: {sentiment_counts.get("negative", 0)}
...
-----------------------------------
Generated by:
Real-Time Market Intelligence & Strategic Insight System
...
with tab_strategy:
    ...
    # Build report text ONCE inside the tab
    ...
    # -----------------------------
    # LEFT: COMPETITIVE INDEX + AI BRIEF + DOWNLOAD
    # -----------------------------
    with c1:
        strength = (
...
<div class="glass glass-hover">
  ...
  <div class="small-note">
    Computed using price momentum, sentiment polarity, forecast direction, and news intensity.
  </div>
</div>
""",
            unsafe_allow_html=True,
        ...
        # ✅ Download button aligned in same column
        st.download_button(
            label="📄 Download Strategic Report",
            data=report_text,
            file_name=f"{company}_Strategic_Report.txt",
            mime="text/plain",
            use_container_width=True,
        )

        # ✅ Explanation should NOT be hidden inside expander
...
""",
        ...
        with st.expander("ℹ️ How is Competitive Index calculated?"):
            st.markdown(
                """
The **Competitive Positioning Index (0–100)** combines:
...
"""
    ...
    # -----------------------------
    # RIGHT: OPPORTUNITY / THREAT SIGNAL
    # -----------------------------
    with c2:
        signal_color = (
...
""",
        ...
        st.markdown(
            f"""
<div class="glass">
  <div style="font-size:1.0rem; line-height:1.55rem; color:#d7d7d7;">
    {interpretation}
  </div>
</div>
""",
